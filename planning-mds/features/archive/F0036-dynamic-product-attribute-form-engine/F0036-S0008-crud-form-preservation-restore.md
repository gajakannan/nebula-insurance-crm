# Story F0036-S0008: Register Controlled CRUD Forms With F0035 Preservation and Close the Canonical Contact-Edit Scenario

## Story Header

**Story ID:** F0036-S0008
**Feature:** F0036 — Form Engine and Form-State Preservation
**Title:** Register Controlled CRUD Forms With F0035 (via the Controlled-Form Dirty-Tracker Adapter) + Restore on Mount; Close F0035 S0003 Contact-Edit Scenario
**Priority:** High
**Phase:** MVP
...
S0007 shipped the controlled-form dirty-tracker (`useControlledDirtyTracker`) and the library-agnostic shared registration helper, and wired every in-scope CRUD form to call the helper while remaining controlled. This story registers those wired forms with F0035 preservation end-to-end and proves the forced-re-auth restore — including the exact canonical scenario F0035 documented but could not deliver: Contact Edit → type into "Notes" → forced re-auth → values restored → explicit re-save. Closing this scenario fully resolves F0035 finding #1 (preservation wired to zero forms). The no-auto-replay mandate from F0035 (plan run 2026-05-23-41109356) applies to every CRUD form. CRUD field-state mechanisms remain unchanged; preservation rides on the controlled-form adapter (ADR-021 §6 reworked 2026-05-27).
...
**Happy Path — All wired forms preserved:**
- **Given** any of the wired controlled CRUD forms in the exhaustive S0007 inventory (broker create/edit, account create/edit incl. account-contact edit, contact create/edit, task create + inline edit, submission create/edit native fields, policy create native fields, renewal create — ~11 components) is dirty (per `useControlledDirtyTracker`)
...
- **And** the form's `initialValues` baseline is updated to the restored values so a "restored but untouched" return state does not re-mark the form dirty unexpectedly
...
- **Given** more than one wired controlled form has a snapshot (distinct `form_key`s)
...
- **Given** the interrupted action was a confirm-dialog/toggle rather than a form (no registered form is dirty)
...
**Snapshot record (F0035 `FormSnapshotRecord`, reused):** `user_id`, `route`, `form_key` (e.g. `contact-edit:<id>`), `form_values` (≤ 256 KB), `dirty_field_paths` (produced by `useControlledDirtyTracker` for CRUD forms / by the RHF adapter for the product-attribute form; in both cases delivered through the shared helper), `snapshot_timestamp`.
...
- All authenticated roles that use the wired controlled forms (BrokerUser experience is identical — any BrokerUser form that fails Save with 401 gets the same preservation). Preservation is not an authorization feature.
...
**Depends On:**
- F0036-S0007 — `useControlledDirtyTracker`, the library-agnostic shared registration helper, and the wired CRUD inventory (controlled forms, no field-state-library change).
...
- Screens involved: the in-scope wired controlled CRUD surfaces (exhaustive inventory in S0007 — ~11 components, create + edit, expanded by the 2026-05-26 PR-M1 sweep), with Contact Edit as the canonical demonstration.
...
**Resolved (grounding / Phase A; refined 2026-05-27):**
- The F0035 API supports this directly (`useSessionRestorableForm` + `consumeFormSnapshot`); the shared helper from S0007 is library-agnostic — for CRUD it bridges `useControlledDirtyTracker` to the `DirtyFormRegistration` contract; for the product-attribute form it bridges RHF. No CRUD form changes field-state library.
...
- [ ] Tests pass (E2E: Contact Edit → Notes → forced re-auth → restore → explicit Save; integration per wired controlled form; unit: per-form_key targeting)

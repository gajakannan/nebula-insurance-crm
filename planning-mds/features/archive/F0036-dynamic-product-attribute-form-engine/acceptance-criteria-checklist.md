...
| S0003 Schema render + AJV parity | ✅ (parity = 0 disagreements) | ✅ (cross-field rules, required empty) | ✅ (host auth; 401/403 upstream) | ✅ (data-schema AJV; cross-field rules via backend `lobErrors[]`) | ✅ (inline AJV msgs; backend authoritative) | ✅ (blocked submit) | ✅ (backend event on host save) | ✅ |
...
| S0007 Controlled-form dirty-tracker + lib-agnostic helper (no CRUD field-state rewrite) | ✅ (per-form parity; equality matrix specified) | ✅ (invalid/missing; one-at-a-time; stable `initialValues` ref; sensitive-field exclusion) | ✅ (each form auth unchanged) | ✅ (no field/validation change; deep-equality contract) | ✅ (server 400/409 surfaces same msg) | ✅ | ✅ (existing create/update event) | ✅ |
...
2. **AJV parity scope** — **✅ RESOLVED (Architect Phase B rework 2026-05-26, ADR-021 §3, was PR-C1 Critical):** client validates the **data-schema** with AJV and parity is measured against the **actual backend** (ADR-022 multiset equality on `(code, pointer)` over the Cyber examples). **Cross-field rules are backend-authoritative**, surfaced via `lobErrors[]` (not a client-evaluated `rules.json`). No backend change; F0036 no longer depends on ADR-023.
3. **Conditional gating mechanism** — **✅ RESOLVED (Architect Phase B rework 2026-05-26, ADR-021 §4, was PR-H1 High):** the presentational enable/disable is owned by the engine's declarative **UI-conditional map** (LOB-adapter frontend config, not the bundle); the validation half is backend-authoritative via `lobErrors[]`. ADR-022's keyword ban applies to bundle data schemas, not this UI-layer map; "no bundle change" holds.
4. **`ui-schema.json` filename** — shipped name is hyphenated; Phase B amends ADR-021 prose.
5. **Workstream B mechanism** — **✅ RESOLVED (operator scope refinement 2026-05-27, ADR-021 §6 reworked):** Workstream B keeps CRUD forms as controlled components and adds `useControlledDirtyTracker` + a library-agnostic shared registration helper. The CRUD field-state mechanism is unchanged; RHF remains scoped to Workstream A. The ~11-component inventory and the F0035 finding #1 closure goal are unchanged.

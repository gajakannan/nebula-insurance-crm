...
| [F0039 — Neuron Durable Conversations & Local Phi Intent Resolution](./F0039-neuron-multi-thread-conversations/README.md) | Neuron Companion | Durable owner-scoped conversation store + thread-management UX **and** a local Microsoft Phi structured scope-and-intent resolver (behind deterministic fail-closed validation) replacing the F0038 mock/deterministic seam. Requirements committed + architecture authored in plan run `2026-07-21-6eeb172f` (9 stories, S0009 gated). Depends on F0038's persistence ADR (ADR-028) + as-built seams. |
